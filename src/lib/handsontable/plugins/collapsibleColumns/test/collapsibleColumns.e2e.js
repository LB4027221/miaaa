describe('CollapsibleColumns', function () {
  ...
  describe('expand headers functionality', function () {
    ...
    it('should expand to master table width after clicking the expand button #105', function () {
      handsontable({
        data: Handsontable.helper.createSpreadsheetData(10, 10),
        rowHeaders: true,
        hiddenColumns: true,
      ...
      var masterWtHider = spec().$container.find('.ht_master .wtHider').first();
      var cloneWtHider = spec().$container.find('.ht_clone_top .wtHider').first();
      var hiderWidthBefore = cloneWtHider.width();

      var button = spec().$container.find('.collapsibleIndicator').first();
