    ...
    return (
      <div className='where-item'>
        ...
        {this.state.expType === 'select'
          ? <Cascader
            ...
            onChange={(value) => {
              ...
              dispatch.having.setWhere({ item, index })
            }}
          />
          : <Input
            style={{
        ...
        <Input
          ...
          value={item.where}
          placeholder={'需要\'号的自行加入'}
          onChange={({ target }) => {
            item.where = [target.value]
            item = { ...item }
            // dispatch(setWhere({ item, index }))
            dispatch.having.setWhere({ item, index })
          }}
        ...
        <span
          ...
          style={{ marginLeft: 5 }}
          onClick={(e) => {
          let _item = componentTemplate.$equals()
          let _index = index + 1

          // dispatch(addWhere({ item: _item, index: _index }))
          dispatch.having.addWhere({ item: _item, index: _index })
        }}
        >插入
        </span>
        {
          index
            ? <span
              ...
              dispatch.having.delWhere({ index })
            }}
            >删除
            </span>
            : ''
