  ...
  if (metaList.length) {
    ...
    if (sqlStr.columns.length && sqlStr.table) {
      ...
      if (groupByComponents && groupByComponents.length) {
        ...
        sqlStr.groupBy = sqlStr.groupBy.filter(i => i)
      }
...
const parserCountSQL = ({
  metaList,
  report: { database, table, limit = 15, offset = 0, target, optGroupBy, groupByComponents, expJoin, regionComponent = [] },
  where,
  joins,
  targets,
  groupBy
  ...
  if (metaList.length && table) {
    ...
    if (sqlStr.columns.length && sqlStr.table) {
      ...
      if (groupByComponents && groupByComponents.length) {
        ...
        groupBy = groupBy.filter(item => item)
      }

      let expression = groupBy.length
...
const convertSelectAlias = item => {
  if (item.alias) return ({ ...item, alias: `\`${item.alias}\`` })
...
const filterAliasList = ({ joins, main }) => R.compose(
  mapIndexed((item, index) => ({ ...item, alias: `${alias[index]}` })),
  R.flatten,
